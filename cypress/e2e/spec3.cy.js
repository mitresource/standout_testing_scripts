...
describe('StandOut Job Listings Test', () => {
  ...
  it('should validate job details from listing to detail page', () => {
    ...
    cy.get("div[class='flex items-center space-x-8'] button:nth-child(1)")
      .should('have.text', 'Jobs')
      .click({ force: true });

    cy.get(
      'body > div:nth-child(14) > main:nth-child(2) > div:nth-child(1) > main:nth-child(2) > section:nth-child(4) > div:nth-child(1) > div:nth-child(2) > div:nth-child(1) > button:nth-child(1) > div:nth-child(2) > h3:nth-child(1)'
    )
      .should('have.text', 'Top 2026 Internships')
      .click({ force: true });

    // Stub window.open before triggering navigation
    cy.window().then((win) => {
    ...
    cy.get('@windowOpen').then((stub) => {
      ...
      cy.get(
        'body > div > main > section > div > div.text-center.w-full.font-semibold > h2'
      )
        .should('be.visible')
      ...
      // ✅ Log all table data (using reusable function)
      table_Elements_Log();
    });
      ...
      .then(($row) => {
        ...
        cy.get('@windowOpen').then((stub) => {
          ...
          const normalizedTitle = companyName
            .toLowerCase()
            .replace(/[^\w]/g, ''); // ✅ removed all special chars
          ...
          cy.get('body', { timeout: 10000 }).then(($body) => {
            ...
            if (salary) {
              ...
              if (salaryNumbers) {
                ...
                expect(pageNumbers, `Expected salary ${salary}`).to.include(
                  salaryNumbers
                );
